 ...
 *
 */
public class CCwantSelectPhotoActivity extends Activity implements SelectCallback, CCwantSelectPhotoAdapter.PhotoCountListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mAdapter.setPhotoCountListener(this);
	...
	public void onSelectListen(String path,boolean isCancel) {
		if(isCancel){
			...
			mPathList.remove(path);
				...
				//Toast.makeText(CCwantSelectPhotoActivity.this,"照片不能超过5张",Toast.LENGTH_SHORT).show();
				mPathList.add(path);
	...
	public int getPhotoCount() {
		...
		Intent intent = getIntent();
		mPhotoCount = intent.getIntExtra("photoCount", 5);
		return mPhotoCount;
	}
}
